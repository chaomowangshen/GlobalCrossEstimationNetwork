...
class PersonNet(nn.Module):
    ...
    def get_X_person_net(self, X_person_net):
        X_person_net = X_person_net.reshape(-1, 1)
        return X_person_net
...
#整合CEN模型
class CEN(nn.Module):
    def __init__(self, n_persons, depth=1, activation='sigmoid'):
        super(CEN, self).__init__()
        self.person_net = PersonNet()
        self.item_net = ItemNet(n_persons, depth, activation)
        self.irt_net = IRTNet()
    def forward(self, x_person,x_item):
        z = self.person_net.get_X_person_net(x_person)
        ab = self.item_net(x_item)
        x = torch.cat([z, ab], dim=1)
        p = self.irt_net(x)
        return p

    def preprocess_data(self, z_csv_path,res_mat):
        n_persons, n_items = res_mat.shape
        # Get the input patterns 'X_person_net' for the person net.
        X_person_net=pd.read_csv(z_csv_path,header=None).values.flatten()
        X_person_net = np.repeat(X_person_net, repeats=n_items, axis=0).reshape(-1, 1)
        X_person_net=torch.tensor(X_person_net,dtype=torch.float32)
...
def train_one_epoch_init(model,z_csv_path,res_mat, optimizer, device,loss_fn,batch_size):
    model.train()
    total_loss = 0
    X_person_net, X_item_net, y_CEN = model.preprocess_data(z_csv_path,res_mat)
...
def evaluate_init(model, res_mat, device, a_true, b_true):
...
def save_metrics(start_time,end_time,times_list,metrics_list,model,res_mat,device,a_true,b_true,depth,activation,n_person,n_item,rep,sim='sim2',res='res2_1'):
    elapsed_time = end_time - start_time
    times_list.append(elapsed_time)
    # Evaluate the model
    model.load_state_dict(torch.load('best_model_init_ab.pth'))
    metric0 = evaluate_init(model, res_mat, device,a_true, b_true)
...
def train_model_init(res_mat,z_csv_path,n_item,n_person,depth,activation,device, epochs, early_stopping_count,batch_size,lr=0.001):
    model= CEN(n_person, depth, activation).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr)
    loss_fn = nn.MSELoss()
    early_stopping = EarlyStopping(patience=early_stopping_count, verbose=False)
    start_time = time()
    for epoch in range(epochs):
        loss = train_one_epoch_init(model,z_csv_path,res_mat, optimizer, device,loss_fn,batch_size)
        if np.isnan(loss):
            print("Loss is NaN. Stopping training.")
            break
        if epoch % 100 == 0:
            print(f'Epoch {epoch}/{epochs}, Loss: {loss}')
        early_stopping(loss, model,filename='best_model_init_ab.pth')
