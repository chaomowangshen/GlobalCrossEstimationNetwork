...
from model.cen_init_ab import CEN,train_one_epoch_init,evaluate_init,save_metrics,train_model_init
from model.cen_qb_ab import CEN_QB,train_one_epoch_qb,evaluate_qb,save_metrics_qb,train_model_qb
import argparse
...
def run(n_person_sim,n_item_sim,n_reps,depths,batch_size,lr,epochs,early_stopping_count,sim='sim2',res='res2_1'):
        ...
        for n_item in n_item_sim:
            ...
            for n_depth in depths:
                ...
                for rep in range(n_reps):
                    print(f"Running simulation2_1 init {rep + 1}/{n_reps} with {n_person} persons and {n_item} items_with {n_depth} depth and {activation} activation")
                    # Load the data
                    ...
                    z_csv_path = f'../data/{sim}/n_p_{n_person}/n_i_{n_item}/rep_{rep}/z.csv'
                    # Initialize the model
                    model,start_time,end_time=train_model_init(res_mat,z_csv_path,n_item,n_person,n_depth,activation,device, epochs, early_stopping_count,batch_size,lr)
                    times_list,metrics_list=save_metrics(start_time,end_time,times_list,metrics_list,model,res_mat,device,a_true,b_true,n_depth,activation,n_person,n_item,rep,sim,res)
                    # Save the results
                timese=np.array(times_list).mean()
                total_time.append(timese)
                metrics_mean=np.array(metrics_list).mean(axis=0)
                metrics_mean=pd.DataFrame(metrics_mean,columns=['MAE','MSE','Bias'],index=['a','b'])
                metrics_std=np.array(metrics_list).std(axis=0)
                metrics_std=pd.DataFrame(metrics_std,columns=['MAE','MSE','Bias'],index=['a','b'])
            ...
            for n_depth in depths:
                ...
                for rep in range(n_reps):
                    ...
                    z_csv_path = f'../data/{sim}/n_p_{n_person}/n_i_{n_item}/rep_{rep}/z.csv'
                    # Initialize the model
                    model,start_time,end_time=train_model_qb(res_mat,z_csv_path,n_person,n_depth,activation,device, epochs, early_stopping_count,lr)
                    times_list,metrics_list=save_metrics_qb(start_time,end_time,times_list,metrics_list,model,res_mat,device,a_true,b_true,n_depth,activation,n_person,n_item,rep,sim,res)
                    # Save the results
                timese=np.array(times_list).mean()
                total_time.append(timese)
                metrics_mean=np.array(metrics_list).mean(axis=0)
